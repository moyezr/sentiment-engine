...
1. Install ollama in your system from https://ollama.com/download
   NEW:
2. Go to `fine_tuning` folder's README.md and follow the instructions to fine-tune the model.
   OLD:
3. Pull the `deepseek-r1:1.5b` model using the comand `ollama pull deepseek-r1:1.5b`.
4. To make sure the model has been pulled, run `ollama run deepseek-r1:1.5b` . If everything is right, you should be able to interact with the model locally.
5. While being inside the `backend` folder, run `ollama create -f Modelfile sentiment-engine-old`. This command creates a model with defined configuration in the `Modelfile` on top of `deepseek-r1:1.5b` with the name of `sentiment-engine`.
6. If everything is right, you should be able to interact with this new model by running `ollama run sentiment-engine`
